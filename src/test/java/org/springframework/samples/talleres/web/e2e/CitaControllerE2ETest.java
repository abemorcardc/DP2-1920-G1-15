...
public class CitaControllerE2ETest {
	...
	void testShowCitaForm() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/citas/{citaId}", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeHasNoErrors("cita"))
			.andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("descripcion", Matchers.is("Problemas con el motor"))))
			.andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("esUrgente", Matchers.is(true)))).andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("tipo", Matchers.is(TipoCita.reparacion))))
			.andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("coste", Matchers.is(120.0)))).andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("tiempo", Matchers.is(40))))
			.andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("estadoCita", Matchers.is(EstadoCita.pendiente)))).andExpect(MockMvcResultMatchers.view().name("citas/citaEnDetalle"));
	}

	// Escenario negativo de un show a una cita
	@WithMockUser(value = "jose", authorities = {
		"cliente"
	})
	@Test
	...
	void testListCliCitaSuccess() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/citas", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.view().name("citas/citaList"));
	...
	void testCancelaCitaSuccess() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/citas/{citaId}/cancelar", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeExists("cita"))
	...
	void testCancelaCitaError() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/citas/{citaId}/cancelar", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().is3xxRedirection())
	...
	void testshowMecCitaDetalle() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/citas/{citaId}", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().isOk())
			...
			.andExpect(MockMvcResultMatchers.model().attribute("cita", Matchers.hasProperty("fechaCita", Matchers.is(LocalDateTime.parse("2021-03-14T13:00")))))
	...
	void testProcessUpdateMecForm() throws Exception {
		this.mockMvc
			.perform(MockMvcRequestBuilders.post("/mecanicos/citas/{citaId}/edit", CitaControllerE2ETest.TEST_CITA_ID).with(SecurityMockMvcRequestPostProcessors.csrf()).param("fechaCita", "22/10/2020 10:00").param("descripcion", "prueba test")
				.param("tiempo", "23").param("coste", "60.0").param("estadoCita", "pendiente"))
	...
	@WithMockUser(value = "pepe", authorities = {
		"mecanico"
	})		//citas pendientes las que no tienen mecanico
	@Test
	...
	@WithMockUser(value = "manolo", authorities = {
		"cliente"
	})		//el cliente no puede aceptar citas
	@Test
	void testCliListaCitasPendientes() throws Exception {

		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/citasPendientes")).andExpect(MockMvcResultMatchers.status().is4xxClientError());
	}

	@WithMockUser(value = "paco", authorities = {
		"mecanico"
	})		//mecanico acepta cita
	@Test
	void testAceptaCitaSuccess() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/citas/{citaId}/aceptar", 4)).andExpect(MockMvcResultMatchers.status().isOk()).andExpect(MockMvcResultMatchers.model().attributeExists("cita"))
	...
	void testAceptaCitaError() throws Exception {
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/citas/{citaId}/aceptar", CitaControllerE2ETest.TEST_CITA_ID)).andExpect(MockMvcResultMatchers.status().is4xxClientError());
