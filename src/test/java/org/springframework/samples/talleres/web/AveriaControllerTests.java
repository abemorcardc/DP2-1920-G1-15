...
class AveriaControllerTests {
	...
	void testShowAveriasList() throws Exception {
		...
		this.mockMvc
				.perform(MockMvcRequestBuilders.get("/mecanicos/vehiculos/{vehiculoId}/averia",
						AveriaControllerTests.TEST_VEHICULO_ID))
	...
	void testShowAveriasListOtroMecanico() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/vehiculos/{vehiculoId}/averia", 3))
				.andExpect(MockMvcResultMatchers.status().isOk())
	...
	@WithMockUser(value = "paco", roles = "mecanico")
	@Test
	void testMecanicoShowAverias() throws Exception {

		// Compruebo que para la cita 1 me devuelve una lista de averias
		BDDMockito.given(this.averiaService.findAveriaById(AveriaControllerTests.TEST_AVERIA_ID))
				.willReturn(this.averia1);
		// Compruebo que para el usuario paco me devuelve su id
		BDDMockito.given(this.mecanicoService.findMecIdByUsername("paco")).willReturn(this.paco.getId());
		// Compruebo que al hacer un GET a /mecanicos/averia/1 no da error y redirije
		// bien
		this.mockMvc
				.perform(MockMvcRequestBuilders.get("/mecanicos/averia/{averiaId}",
						AveriaControllerTests.TEST_AVERIA_ID))
				.andExpect(MockMvcResultMatchers.status().isOk())
	...
	void testShowAveriasOtroMecanico() throws Exception {
		...
		// Compruebo que para el usuario paco me devuelve su id
		BDDMockito.given(this.mecanicoService.findMecIdByUsername("paco")).willReturn(this.paco.getId());
		// Compruebo que al hacer un GET a /mecanicos/averia/2 no da error y redirije
		// bien
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/averia/{averiaId}", 2))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.view().name("exception"));
	}
	...
	// // Compruebo que para la cita 1 me devuelve una lista de averias
	//
	// //BDDMockito.given(this.averiaService.findAveriasByVehiculoId(this.mercedes.getId())).willReturn(Lists.newArrayList(this.averia1,
	// new Averia()));
	//
	// // Compruebo que al hacer un GET a /averias/averiasDeVehiculoList no da error
	// y redirije bien
	// this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/{vehiculoId}",
	// AveriaControllerTests.TEST_VEHICULO_ID_404)).andExpect(MockMvcResultMatchers.status().is4xxClientError())
	// .andExpect(MockMvcResultMatchers.view().name("averias/MecAveriasDeVehiculoList"));
	//
	// }
	...
	// Tests Historia 6 (Abraham y David) ------------------
	// show averia del cliente:
	@WithMockUser(value = "manolo", roles = "cliente")
	@Test
	void testClienteShowAverias() throws Exception {

		// Compruebo que para la cita 1 me devuelve una lista de averias
		BDDMockito.given(this.averiaService.findAveriaById(AveriaControllerTests.TEST_AVERIA_ID))
				.willReturn(this.averia1);
		// Compruebo que para el usuario manolo me devuelve su id
		BDDMockito.given(this.mecanicoService.findMecIdByUsername("manolo")).willReturn(this.manolo.getId());
		// Compruebo que al hacer un GET a /cliente/averia/1 no da error y redirije
		// bien
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/averia/{averiaId}", 1))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.view().name("averias/ClienteAveriaShow"));
	}

	// show averia de otro cliente:
	@WithMockUser(value = "manolo", roles = "cliente")
	@Test
	void testClienteShowAveriasError() throws Exception {
		...
		// Compruebo que para el usuario manolo me devuelve su id
		BDDMockito.given(this.mecanicoService.findMecIdByUsername("manolo")).willReturn(this.manolo.getId());
		// Compruebo que al hacer un GET a /cliente/averia/2 no da error y redirije
		// bien, en este caso a la vista exception
		this.mockMvc.perform(MockMvcRequestBuilders.get("/cliente/averia/{averiaId}", 2))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.view().name("exception"));
	}
	...
	void testInitCreationForm() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/{vehiculoId}/new", 1))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.model().attributeExists("averia"))
	...
	void testInitCreationFormOtroMecanico() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get("/mecanicos/{vehiculoId}/new", 3))
				.andExpect(MockMvcResultMatchers.status().isOk())
				.andExpect(MockMvcResultMatchers.model().attributeExists("averia"))
				.andExpect(MockMvcResultMatchers.view().name("exception"));
	}
	...
	void testProcessAverCreateFormNoCita() throws Exception {
		this.mockMvc
				.perform(MockMvcRequestBuilders
						.post("/mecanicos/{vehiculoId}/new", AveriaControllerTests.TEST_VEHICULO_ID)
						.param("nombre", "limpiaparabrisas")
						.param("descripcion",
								"Fallo en el limpiaparabrisas, no funciona la opcion rapida, cambio de piezas")
						.with(SecurityMockMvcRequestPostProcessors.csrf()).param("coste", "50.0").param("tiempo", "20")
						.param("piezasNecesarias", "5").param("complejidad", "BAJA"))
				.andExpect(MockMvcResultMatchers.view().name("redirect:/mecanicos/{vehiculoId}/citas"));
	...
	void testProcessAverCreateForm() throws Exception {
		this.mockMvc
				.perform(MockMvcRequestBuilders
						.post("/mecanicos/{vehiculoId}/new", AveriaControllerTests.TEST_VEHICULO_ID, "1")
						.param("nombre", "limpiaparabrisas")
						.param("descripcion",
								"Fallo en el limpiaparabrisas, no funciona la opcion rapida, cambio de piezas")
						.with(SecurityMockMvcRequestPostProcessors.csrf()).param("coste", "50.0").param("tiempo", "20")
						.param("piezasNecesarias", "5").param("complejidad", "BAJA").queryParam("citaId", "1"))
				.andExpect(MockMvcResultMatchers.view().name("redirect:/mecanicos/vehiculos/{vehiculoId}/averia"));

	}

	// Si uno de los parámetros está vacío, por ejemplo el nombre, redirige bien a
	// la pagnia excepcion
	@WithMockUser(value = "paco", roles = "mecanico")
	@Test
