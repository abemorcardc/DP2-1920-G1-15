...
class MecMuestraCita extends Simulation {
	...
	object CitasShowError {
		...
		exec(http("CitaShowError")
			.get("/mecanicos/citas/1")
			.headers(headers_30)
			.resources(http("CitaShowErrorResources")
			.get("/mecanicos/citas/1")
			.headers(headers_0)))
		.pause(21)
	}

	val PacoScn = scenario("Paco").exec(Home.home, LoginPaco.login, CitasList.citasList, CitasShow.citasShow)

	val LoloScn = scenario("Lolo").exec(Home.home, LoginLolo.login, CitasList.citasList, CitasShowError.citasShow)
